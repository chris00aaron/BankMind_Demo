...
public class DailyWithdrawalPredictionService {
    ...
    /**
     * Este método se encarga de obtener las predicciones de retiros de efectivo en
     * cajeros automáticos
     * para una fecha específica, en caso no existan predicciones para la fecha
     * especificada, se generan
     * 
     * @param fecha La fecha para la cual se quieren obtener las predicciones
     * @return Una lista de DailyWithdrawalPrediction que contiene las predicciones
    ...
    public List<DailyWithdrawalPrediction> obtenerPrediccionesPorFecha(LocalDate fecha) {
        ...
        List<DailyAtmTransaction> transaccionesAyer = dailyAtmTransactionService
                .obtenerTransaccionesPorFecha(fechaAyer);

        // El .stream().toList() es inmutable y seguro contra
