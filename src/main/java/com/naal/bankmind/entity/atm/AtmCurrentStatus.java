...
public class AtmCurrentStatus {
    ...
    @MapsId // Indica que la clave primaria de esta entidad es la misma que la de la entidad relacionada
    @JoinColumn(name = "id_atm", // Columna en la tabla atm_current_status (FK)
            referencedColumnName = "id_atm", // Columna en la tabla atms (PK)
            foreignKey = @ForeignKey(name = "fk_atm_current_status-atm"), // Nombre de la restricción de clave foránea
    ...
    @JoinColumn(name = "last_sync_id", // Columna en la tabla atm_current_status (FK)
            ...
            foreignKey = @ForeignKey(name = "fk_atm_current_status-sync_log") // Nombre de la restricción de clave foránea
    )
